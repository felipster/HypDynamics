function Xdot = fourBarLinkSusp(t,X,S,A,F)
     % extract constant variables from main script
     [y1_0, L1, th1_0, Ls2, Am, w0,...
      k2_r] = v2struct(S);
     % extract state variables from ode45 things
    ...
    % Conditional for spring force: only applies spring like behavior when in compression 
    ...
    % substitute and evaluate into governing equation matrix
    A = subs(A);
    A = eval(A);
    F = subs(F);
    F = eval(F);
